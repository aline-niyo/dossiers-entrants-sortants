...
if (isset($_POST['valider'])) {
    ...
    $insertquest = $conn->prepare("INSERT INTO questionnairedemande (id_utilisateur, date_naissance, province, school, father, mother) VALUES (:utilisateur, :date_naissance, :province, :school, :father, :mother)");

    $insertquest->bindParam(':utilisateur', $recupuser);
    $insertquest->bindParam(':date_naissance', $recupedate);
    $insertquest->bindParam(':province', $recupepro);
    $insertquest->bindParam(':school', $recupscho);
    $insertquest->bindParam(':father', $recufat);
    $insertquest->bindParam(':mother', $recumot);

    // Check if the user already exists
    $checkUser = $conn->prepare("SELECT COUNT(*) FROM questionnairedemande WHERE id_utilisateur = :utilisateur");
    $checkUser->bindParam(':utilisateur', $recupuser);
    $checkUser->execute();
    $userExists = $checkUser->fetchColumn();

    if ($userExists > 0) {
        echo "<script>alert('Désolé, l\'utilisateur existe déjà');</script>";
    } else {
